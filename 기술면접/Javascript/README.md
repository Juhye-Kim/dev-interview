...
19. event
    - DOM 이벤트가 어떻게 전달되는지 설명해주세요.
      - 이벤트 버블링
        - 자식노드 → 부모노드 순으로 이벤트를 실행
        - DOM트리 위쪽으로 올라가면서 전파
      - 이벤트 캡쳐링
        - 부모노드 → 자식노드 순으로 이벤트를 실행
        - DOM트리 아래쪽으로 내려가면서 전파
    - 이벤트 위임, 전파
      - 여러개의 DOM 요소에 각각 이벤트핸들러를 등록하는 대신, 하나의 상위 DOM 요소에만 등록하는 방법
      - **장점**
        - 많은 핸들러를 할당하지 않아도 됨 → 초기화 단순, 메모리 절약
        - 요소를 추가/제거시 해당 요소에 할당된 핸들러를 추가/제거할 필요가 없어 코드가 짧아짐
      - **단점**
        - 이벤트가 반드시 버블링 되어야 하는데, 몇몇 이벤트는 버블링 되지 않음
        - 낮은 레벨에 할당한 핸들러엔 `event.stopPropagation()`를 쓸 수 없음
        - CPU 작업 부하가 늘어날 수 있음
          - 컨테이너의 핸들러는 모든 하위 컨테이너에서 발생하는 이벤트에 무조건 응답해야 해서
            - 무시할만한 수준이기는 함
      - 이벤트 전파 이용
        - 이벤트가 발생한 요소부터 상/하위로 (DOM 트리를 통해) 이벤트가 전파되는 것
          - 캡쳐링 = 상위요소 → 하위요소
          - 버블링 = 하위요소 → 상위요소
        - 이벤트 전파 방지 → event 객체의 stopPropagation 메서드 이용
          - preventDefalut는 DOM요소의 기본 동작을 막음
      - 이벤트 캡쳐링은 어떤 경우에 쓸 수 있을까요?
        - 버블링이 일어나지 않는 이벤트들
          - event.bubbles = false
            - 포커스 이벤트: focus/blur
            - 리소스 이벤트: load/unload/abort/error
            - 마우스 이벤트: mouseenter/mouseleave
        - 상위에서 위 이벤트를 캐치하려면 캡처링 때 캐치해야함!
          - focus/blur 이벤트 → focusin/focusout
          - mouseenter/mouseleave → mouseover/mouseout로 대체
          - focusin/focusout, mouseover/mouseout → 버블링으로 전파됨 → 캡처링에서 이벤트를 캐치해야 할 경우는 거의 없음
    - 이벤트 핸들러 프로퍼티 방식, addEventListener 방식의 차이점?
      - 이벤트 핸들러 프로퍼티 방식 (.onclick)
        - 이벤트 핸들러 하나만 등록 가능
      - addEventListener
        - 여러개 이벤트핸들러 등록 가능
          - 등록된 순서대로 호출됨!
        - 이벤트 전파 방식 지정 가능 (세번째 인자)
          - false면 버블링, true면 캡처링
    - document.load(onload), DOMContentLoaded 이벤트의 차이점?
      - **DOMContentLoaded 이벤트**
        - HTML 문서 로딩, 파싱이 끝나고 DOM 생성이 완료되었을 때 발생
          - 스타일시트, 이미지, 폰트..등 리소스 로딩은 기다리지 X
      - **load 이벤트**
        - DOMContentLoaded 발생 후, 모든 리소스(이미지, 폰트..) 로딩이 완료되었을 때 발생
    - 변수 선언, 초기화, 할당의 차이점
      - **선언**
        - 값이 저장될 빈 공간을 메모리에 확보
          - 런타임 이전에 실행됨
        - 변할 수 있는 데이터를 만들고자 하는데, 이 데이터의 주소 이름는 apple로 할거다 let apple;
      - **할당**
        - 선언된 변수에 값을 저장하는 것
        - 할당 명령을 하면 컴퓨터는 apple이라는 주소를 검색해 그곳에 3이라는 데이터를 넣는다.
          - 사실 직접 저장하는 것은 아니고, 다른 곳에 데이터를 저장하고 그곳의 주소만 저장
          - 런타임에 실행
        - 'apple 이라는 이름을 가진 주소에, 3이라는 데이터를 저장할거다'
      - **초기화**
        - 변수 선언 후 처음으로 값을 저장하는 것
        - 초기화를 하지 않으면, 확보된 공간에는 '쓰레기값'이라고 하는 다른 어플리케이션이 사용했던 값이 남아있을 수도 있음
          - var 키워드를 쓰면 undefined로 암묵적으로 초기화를 해줌
    - Call by value, Call by reference에 대해 설명하시오.
      - **Call by value**
        > 값에 의한 호출
        - 전달되는 변수 값을 복사해서 전달
          - primitive type은 call by value 호출 방식 사용
        - 장점 : 함수 안에서 인자 값이 변경되어도, 외부 변수 값은 변경되지 X
          - 복사된 인자는 함수 안에서 지역적으로 사용됨 → local value 속성을 가짐
        - 단점 : 값 복사 → 메모리 공간 낭비
      - **Call by reference**
        > 참조에 의한 호출
        - 함수 호출 시 인자로 주소를 넘겨줌
          - reference type은 call by reference 호출 방식 사용
        - 장점 : 복사X → 메모리 공간 낭비 줄어듦
        - 단점 : 주소 참조 → 함수 안에서 인자 값이 변경되면, 전달된 객체 값도 변경됨
20. js의 Heap, Stack의 차이점
    - Stack
      - 함수 호출과 관계되는 지역 변수, 매개변수가 저장됨
        - 함수 호출과 함께 할당, 호출 완료시 소멸
      - 실행 컨텍스트가 추가, 제거되는 영역
      - 컴파일 타임에 크기 결정
        - 메모리 주소가 높은 영역부터 채워짐
    - Heap
      - 사용자에 의해 동적으로 할당, 해제되는 메모리 영역
      - 객체가 저장되는 메모리 공간
        - 참조타입은 heap에 (힙의 주소만 stack에)
          - 런타임에 동적으로 크기가 정해지기 때문
      - 런타임에 크기 결정
        - 메모리 주소가 낮은 영역부터 채워짐
